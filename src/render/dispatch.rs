...
pub(crate) fn render_recursive(
    ...
    let output = match cli.output_mode() {
        ...
        OutputMode::Text if cli.docs => render::docs::render_docs_recursive(&items, root_path),
            ...
            let output = match cli.output_mode() {
                OutputMode::Json => {
                    let items: Vec<&types::IndexItem> =
                        indices.iter().map(|&i| index.get(i)).collect();
                    render::json::render_json_ambiguous(&items)
                }
                OutputMode::Brief | OutputMode::Text => {
                    render::ambiguous::render_ambiguous(index, indices, query)
                }
            };
            writeln!(w, "{output}").map_err(GroxError::Io)?;
            Ok(())
